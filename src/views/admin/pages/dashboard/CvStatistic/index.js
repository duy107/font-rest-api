        ...
        <>
            <div className="w-full h-full bg-cyan-400 rounded-md py-4 cursor-pointer hover:scale-105 transition-transform duration-500 ease-in-out">
                ...
                <div class="mt-3 flex flex-col gap-2 text-center text-white">
                    <span class="text-[15px]">Số lượng CV: <strong>{cvStatistic.total}</strong></span>
                    <span class="text-[15px]">Số CV đã đọc: <strong>{cvStatistic.statusTrue}</strong></span>
                    <span class="text-[15px]">Số CV đã chấp nhận: <strong>{cvStatistic.statusAccept}</strong></span>
                </div>
            </div>
        </>
    );
